...
def vaga(titulo: str, local: str = "Joinville, SC", modalidade: str = "Híbrido") -> Job:
    return Job(
        titulo=titulo,
...
def test_vaga_remota_internacional_passa():
    assert vaga("iOS Developer", "Remote - US only", "Remoto").combina_com(PERFIL_IOS.regras)


def test_vaga_em_joinville_passa():
    assert vaga("iOS Developer", "Joinville, SC", "Híbrido").combina_com(PERFIL_IOS.regras)
